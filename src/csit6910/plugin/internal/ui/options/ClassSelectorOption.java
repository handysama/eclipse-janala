...
public class ClassSelectorOption extends Option implements IOptionChangeListener {
    ...
    fClassAddFromClasspaths.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        try {
          ...
          IStatus s = PluginStatus.JAVA_MODEL_EXCEPTION.getStatus(jme);
          Activator.log(s);
        }
      }
    });
  ...
  public IStatus isValid(ILaunchConfiguration config) {
    ...
    List<String> grayedTypesMnemonic = ArgumentCollector.getGrayedTypes(config);
    List<String> selectedTypeMnemonics = ArgumentCollector.getCheckedTypes(config);

    if (selectedTypeMnemonics.isEmpty()) {
      return PluginStatus.createUIStatus(IStatus.ERROR, "No classes or methods selected");
    }
    ...
    for (String typeMnemonicString : selectedTypeMnemonics) {
      ...
      if (fJavaProject == null || !fJavaProject.equals(typeMnemonic.getJavaProject()) || !typeMnemonic.exists()) {
        return PluginStatus.createUIStatus(IStatus.ERROR, "One of the classes does not exist in the selected project");
      }
      
      if (grayedTypesMnemonic.contains(typeMnemonicString)) {
        List<IMethod> methodList = new ArrayList<IMethod>();
        
        List<?> selectedMethods = ArgumentCollector.getCheckedMethods(config, typeMnemonicString);
        for (Object methodObject : selectedMethods) {
          ...
          if (m == null) {
            return PluginStatus.createUIStatus(IStatus.ERROR, "One of the methods does not exist");
          } else if (!m.exists()) {
            String msg = NLS.bind("Mmethod '{0}' does not exist", m.getElementName());
            return PluginStatus.createUIStatus(IStatus.ERROR, msg);
          }
  ...
  public void initializeWithoutListenersFrom(ILaunchConfiguration config) {
    ...
    List<String> availableTypes = ArgumentCollector.getAvailableTypes(config);
    List<String> grayedTypes = ArgumentCollector.getGrayedTypes(config);
    List<String> checkedTypes = ArgumentCollector.getCheckedTypes(config);
    ...
    for (String typeString : availableTypes) {
      ...
      List<String> checkedMethods = ArgumentCollector.getCheckedMethods(config, typeString);
      if (typeMnemonic.getType() != null) {
        ...
        List<String> availableMethods = ArgumentCollector.getAvailableMethods(config, typeString);
  ...
  public void performApply(ILaunchConfigurationWorkingCopy config) {
    ...
    ArgumentCollector.saveClassTree(config, availableTypes, checkedTypes, grayedTypes,
  ...
  public void setDefaults(ILaunchConfigurationWorkingCopy config) {
    ...
    ArgumentCollector.setAvailableTypes(config, emptyList);
    ArgumentCollector.setCheckedTypes(config, emptyList);
    ArgumentCollector.setGrayedTypes(config, emptyList);
    
    List<String> availableTypes = ArgumentCollector.getAvailableTypes(config);
    for (String typeMnemonic : availableTypes) {
    	ArgumentCollector.deleteAvailableMethods(config, typeMnemonic);
    	ArgumentCollector.deleteCheckedMethods(config, typeMnemonic);
    }
  }
        ...
        for (Object obj : selection) {
          try {
            if (obj instanceof IType) {
              ...
              return PluginStatus.createUIStatus(IStatus.ERROR, "One of the selected elements is not a Java class or enum");
            }
          } catch (JavaModelException e) {
            IStatus s = PluginStatus.JAVA_MODEL_EXCEPTION.getStatus(e);
            Activator.log(s);
