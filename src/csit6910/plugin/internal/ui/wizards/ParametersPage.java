...
import csit6910.plugin.internal.ui.options.ClassSelectorOption;
import csit6910.plugin.internal.ui.options.OptionFactory;
import csit6910.plugin.internal.ui.options.OutputDirectoryOption;
...
public class ParametersPage extends OptionWizardPage {
  ...
  protected ParametersPage(String pageName, IJavaProject project, ILaunchConfigurationWorkingCopy config) {
    super(pageName, "Janala Launch Configuration Parameters", RandoopPluginImages.DESC_WIZBAN_NEW_RNDP, config);
...
//    options.addAll(OptionFactory.createPalusOptionGroup(comp, getBasicoptionChangeListener()));
    options.addAll(OptionFactory.createJanalaOptionGroup(comp, getBasicoptionChangeListener()));
    SWTFactory.createHorizontalSeperator(comp, 1);
    
    fTestInputSelectorOption = new ClassSelectorOption(comp, getWizard().getContainer(), fProject);
...
//    options.addAll(OptionFactory.createAdvancedOptionGroup(comp, getBasicoptionChangeListener()));
  ...
  private void createResourcesComposite(Composite parent) {
    ...
    SWTFactory.createLabel(comp, PluginMessages.RandoopOption_junit_output_dir, 1);
    ...
    SWTFactory.createLabel(comp, PluginMessages.RandoopOption_junit_package_name, 1);
    Text packageNameText = SWTFactory.createSingleText(comp, 2);

    //csit6910: modified here
//    SWTFactory.createLabel(comp, PluginMessages.RandoopOption_junit_classname, 1);
