package csit6910.plugin.internal.core;
...
public class PluginStatus {
   ...
   * @param message
   */
  private PluginStatus(int code, int severity, String message) {
    fCode = code;
    fSeverity = severity;
    fMessage = message;
  }
  ...
  public IStatus getStatus() {
    ...
    return new Status(fSeverity, Activator.getPluginId(), fCode, m, null);
  }

  public IStatus getStatus(Throwable t) {
    String m = MessageFormat.format(fMessage, new Integer(fCode));
    return new Status(fSeverity, Activator.getPluginId(), fCode, m, t);
  }
  ...
  // csit6910
  public static final PluginStatus NO_LOCAL_CATGCONF_ERROR =
      new PluginStatus(2001, IStatus.ERROR,
          "Error while retreiving CATG.conf (code {0}).");

  public static final PluginStatus NO_LOCAL_IAGENTJAR_ERROR =
      new PluginStatus(2002, IStatus.ERROR,
          "Error while retreiving iagent archive (code {0}).");
  
  public static final PluginStatus NO_LOCAL_RANDOOPJAR_ERROR =
    new PluginStatus(1001, IStatus.ERROR,
        "Error while retreiving Randoop archive (code {0}).");

  public static final PluginStatus COMM_NO_FREE_PORT =
    new PluginStatus(5000, IStatus.ERROR,
        "Randoop plug-in could not find a free communication port. Check your firewall settings (code {0}).");
  
  public static final PluginStatus COMM_TERMINATED_SESSION =
    new PluginStatus(5001, IStatus.ERROR,
        "Randoop closed communications on port {1} unexpectadely. Was the process killed? (code {0}).");
  
  public static final PluginStatus COMM_MESSAGE_CLASS_NOT_FOUND =
    new PluginStatus(5002, IStatus.ERROR,
        "Randoop plug-in received a message from Randoop with an Object whose class is not in the plug-ins classpath (code {0}).");
  
  public static final PluginStatus GENERATED_FILE_NOT_IN_PROJECT =
    new PluginStatus(10000, IStatus.ERROR,
        "The JUnit test file {1} is not in the project for which it was generated (code {0}).");
  
  public static final PluginStatus JAVA_MODEL_EXCEPTION =
    new PluginStatus(10001, IStatus.ERROR,
  ...
  public static final PluginStatus PART_INIT_EXCEPTION =
    new PluginStatus(10001, IStatus.ERROR,
    "The view part could not be created or initialized. Try again or open the view by hand (code {0})");
  
  public static final PluginStatus RESOURCE_REFRESH_EXCEPTION =
    new PluginStatus(10002, IStatus.ERROR,
        "An error occured while trying to refresh a resource. Try running the operation again (code {0})");

  public static final PluginStatus NO_JAVA_PROJECT =
    new PluginStatus(20003, IStatus.ERROR,
  ...
  public static IStatus createUIStatus(int severity, String message) {
    return new Status(severity, Activator.getPluginId(),
  ...
  public static IStatus createLaunchConfigurationStatus(int severity, String message, Throwable exception) {
    return new Status(severity, Activator.getPluginId(), RANDOOP_LAUNCH_CONFIGURATION,
        message, exception);
  }
